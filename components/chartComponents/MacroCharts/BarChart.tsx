...
const BarChart = ({goal, progress}: BarChartProps) => {
    ...
    const endProtein = useDerivedValue(() => Math.min(((progressDaily.value.protein / goal.calories) * width * 4 ), width));
    const endCarb = useDerivedValue(() => Math.min(goal.calories ? (progressDaily.value.carbs / goal.calories) * width * 4 + endProtein.value: 0, width));
    const endFat = useDerivedValue(() => Math.min(goal.calories ? (progressDaily.value.fat / goal.calories) * width * 9 + endCarb.value : 0, width));
