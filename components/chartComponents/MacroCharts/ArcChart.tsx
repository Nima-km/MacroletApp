...
const ArcChart = ({ targetPercentage, dailyProgress }: ArcProgressProps) => {
    ...
    const radius =
        ...
        STROKE_WIDTH / 2 -
        (8.25 / 80) * STROKE_WIDTH * 2 -
        smaller_chart;
    if (!targetPercentage) targetPercentage = 1;
    ...
    const animateChart = () => {
        ...
        percentageRemaining.value = withTiming(
            targetPercentage - dailyProgress,
            {
                duration: 1250,
                easing: Easing.inOut(Easing.cubic),
            },
    ...
    const offset =
        outerRadius -
        Math.sqrt(outerRadius * outerRadius * 2) / 2 -
        (STROKE_WIDTH / 2) * (1 - Math.sqrt(2) / 2) -
        smaller_chart;
    const angle_change = (180 / (Math.PI * radius)) * STROKE_WIDTH;
    const consumedText = useDerivedValue(
        () => processNumber(percentageConsumed.value).toString() + " cal",
    );
    const remainingText = useDerivedValue(() =>
        processNumber(percentageRemaining.value).toString(),
    );
    const targetText = useDerivedValue(() =>
        processNumber(targetPercentage).toString(),
    );
    const boundryRect = {
        x: 0,
        y: 0,
    ...
    const end = useDerivedValue(
        () => percentageConsumed.value / targetPercentage,
    );
    const widthConsumed = useDerivedValue(
        () => width / 2 - h1.measureText(consumedText.value).width / 2,
    );
    const widthRemaining = useDerivedValue(
        () =>
            h5.measureText("Remaining").width / 2 -
            h1.measureText(remainingText.value).width / 2,
    );

    const widthTarget = useDerivedValue(
        () => width - h2.measureText(targetText.value).width,
    );
