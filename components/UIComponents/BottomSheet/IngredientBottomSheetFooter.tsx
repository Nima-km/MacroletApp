...
import { useRecipeDraftStore, useRecipeStateStore } from '@/store/recipeStore/useRecipeStore';
...
const IngredientBottomSheetFooter = ({onLogAll, isNewRecipe} : Props) => {
    ...
    const {data: newRecipe, addIngredient : addIngredientNewRecipe} = useRecipeDraftStore()
    const {data: recipe, addIngredient: addIngredientRecipe} = useRecipeStateStore()
    ...
    function onButton() {
        if (foodFullData) {
            ...
            isNewRecipe ? addIngredientNewRecipe(foodFullData) : addIngredientRecipe(foodFullData)
            onLogAll()
        }
    }
    return (
    <View style={{backgroundColor: colors.off_white, padding: 16, gap: 20}}>
        ...
        <View style={[styles.rowContainer, {marginBottom: 20}]}>
            ...
            <View style={styles.chartsContainer}>
                <SimpleChartProteinSmall target={total_macro} progress={(foodData.protein)} backgroundColor={colors.white} />
                <SimpleChartCarbsSmall target={total_macro} progress={(foodData.carbs)} backgroundColor={colors.white} />
                <SimpleChartFatSmall target={total_macro} progress={(foodData.fat)} backgroundColor={colors.white} />
            </View>
        </View>
        <PrimaryButton onPress={() => onButton()}>
            Log All
        </PrimaryButton>
    </View>
    )
