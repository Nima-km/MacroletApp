...
const PrepCookButton = ({
    ...
    const [newPrepHour, setNewPrepHour] = useState(
        getHour(prepTime).toString(),
    );
    const [newPrepMinute, setNewPrepMinute] = useState(
        getMinute(prepTime).toString(),
    );
    const [newCookHour, setNewCookHour] = useState(
        getHour(cookTime).toString(),
    );
    const [newCookMinute, setNewCookMinute] = useState(
        getMinute(cookTime).toString(),
    );
    useEffect(() => {
    ...
    return (
        <View style={styles.container}>
            <Pressable
                ...
                onPress={() => setPrepModalVisible(!prepModalVisible)}
                disabled={disable}
            >
                <H4>Prep</H4>
                <H5>
                    {getHour(prepTime) > 0 && getHour(prepTime) + " hr "}
                    {(getMinute(prepTime) > 0 || disable) &&
                        getMinute(prepTime) + " mins"}
                    {getHour(prepTime) == 0 &&
                        getMinute(prepTime) == 0 &&
                        !disable && (
            ...
            <Pressable
                style={styles.subSection}
                onPress={() => setCookModalVisible(!prepModalVisible)}
                disabled={disable}
            >
                <H4>Cook</H4>
                <H5>
                    {getHour(cookTime) > 0 && getHour(cookTime) + " hr "}
                    {(getMinute(cookTime) > 0 || disable) &&
                        getMinute(cookTime) + " mins"}
                    {getHour(cookTime) == 0 &&
                        getMinute(cookTime) == 0 &&
                        !disable && (
                            <H5 style={{ color: colors.inactive }}>
                                Enter Time
                            </H5>
            ...
            <Pressable style={styles.subSection} disabled={disable}>
                ...
                <H5>
                    {calculateHour(
                        getHour(prepTime) + getHour(cookTime),
                        getMinute(prepTime) + getMinute(cookTime),
                    ) > 0 &&
                        calculateHour(
                            getHour(prepTime) + getHour(cookTime),
                            getMinute(prepTime) + getMinute(cookTime),
            ...
            >
                ...
                >
                    <PrepCookSetter
                        hour={newPrepHour}
                        mins={newPrepMinute}
                        setMinute={setNewPrepMinute}
                        setHour={setNewPrepHour}
                        onSave={setPrepTime}
                        onModal={() => setPrepModalVisible(false)}
                    />
                </ModalCore>
            </Modal>
            <Modal
            ...
            >
                <ModalCore
                    title="Cook Time"
                    onClose={() => setCookModalVisible(false)}
                >
                    <PrepCookSetter
                        hour={newCookHour}
                        mins={newCookMinute}
                        setMinute={setNewCookMinute}
                        setHour={setNewCookHour}
                        onSave={setCookTime}
                        onModal={() => setCookModalVisible(false)}
                    />
                </ModalCore>
            </Modal>
        </View>
