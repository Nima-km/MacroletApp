...
import { addStep, deleteStep, updateStep } from '@/helper/recipeHelpers/stespUtil';
...
const DirectionsEdit = ({recipeObject}: Props) => {
    const [directions, setDirections] = useState<DirectionStep[]>(recipeObject.data.recipeData.directions ?? []);
    ...
    return (
        <View style={{flex: 1}}>
            <FlatList
                ...
                renderItem={({item, index}) => 
                    <StepsCardEdit 
                        item={item} 
                        index={index} 
                        onChangeText={(text) => setDirections(updateStep(directions, item.id, {text: text}))}
                        onDelete={() => setDirections(deleteStep(directions, item.id))}
                    />
            ...
            <SecondaryAddButton onPress={() => setDirections(addStep(directions, {id: generateId(), text: '', photos: []}))} >
                Add Step
            </SecondaryAddButton>
        </View>
    )
